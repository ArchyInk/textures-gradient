  ...
  var $ = function $(selection) {
    ...
    if (linearGradient) {
      ...
      for (var i in linearGradient) {
        ...
        var chunk = defs.append("pattern").attr("id", "chunk_" + i).attr("patternUnits", "objectBoundingBox").attr("width", 1).attr("height", 1).attr("x", i == 0 ? "0" : box.width * (linearGradient[i - 1].stop - linearGradient[i - 1].start) / 100);
        ...
        rect.attr("width", box.width * rate).attr("height", box.height).attr("x", i > 0 ? box.width * (linearGradient[i - 1].stop - linearGradient[i - 1].start) / 100 : "0").style("fill", "url(#chunk_".concat(i, ")"));
      }
    } else {
